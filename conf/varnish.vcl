...
sub vcl_recv {
  ...
  # 不同的HOST选择不同的backend
  if(req.http.host == "dashboard.vicanso.com"){
    set req.backend_hint = dashboard.backend();
  }elsif(req.http.host == "docs.vicanso.com"){
    set req.backend_hint = docs.backend();
  }
...
sub vcl_init{
  ...
  dashboard.add_backend(dashboard0)
  new docs = directors.round_robin();
  docs.add_backend(docs0)
}