...
module.exports = function(config){
  var backendConfig = getBackendConfig(config.backends);
  var initConfig = getInitConfig(config.backends);
  var backendSelectConfig = getBackendSelectConfig(config.backends);
  ...
  fs.writeFileSync(path.join(config.path, 'varnish.vcl'), compiled(varnishConfig));
};
