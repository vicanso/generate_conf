...
module.exports = function(config){
  ...
  haproxyConfig.aclConfig = getAclConfig(config.backend);
  var backends = _.extend({}, config.backend);
  backends.varnish = config.varnish;
  haproxyConfig.backendConfig = getBackendConfig(backends);
  fs.writeFileSync(path.join(paths.config, haproxyConfig.name + '.cfg'), compiled(haproxyConfig));
};
